...
are 12 parameter sets so the entire parameter space can be run by running the 
...
* ...
* `sbatch bash/run_analysis.sh 10`
* `sbatch bash/run_analysis.sh 11`
* `sbatch bash/run_analysis.sh 12`
...
simulations and model fitting sequantially does not scale well for the number
...
done by using the `post_process_daisie_results()` function.
